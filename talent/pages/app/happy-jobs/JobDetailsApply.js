...
export function JobRoleTitleLink({ data, className = '', children }) {
    ...
    const handleClick = (e) => {
        e.preventDefault();
        const applyBtn = document.getElementById(`${data.enc_id}+singleOppAppyBtn`);
        if (applyBtn) {
            applyBtn.click();
        }
    };
    ...
    return (
        <a
            href={data.aggregator_application_link}
            target="_blank"
            rel="noopener noreferrer"
            className={`job-role-title-link ${className}`.trim()}
            style={{ color: 'inherit', display: 'inline-flex', alignItems: 'center', gap: '0.25rem', textDecoration: 'none' }}
            onClick={handleClick}
        >
            ...
            <span className="aggregator-apply-link" style={{ display: 'inline-flex' }}>
                <svg style={{ width: '1.5rem', height: '1.5rem' }} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
...
export default function JobDetailsApply({ data, setIsHeaderVisible, handleCustomizeResume, hideApplyButton = false }) {
    ...
    return (
        <>
            ...
            {hideApplyButton && data.aggregator_application_link &&
                <button
                    type="button"
                    className="applyBtn"
                    id={`${data.enc_id}+singleOppAppyBtn`}
