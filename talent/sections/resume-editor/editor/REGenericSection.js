...
export default function REGenericSection({ expanded, setExpanded, resumeJson, setResumeJson, sectionKey }) {
    const [formValue, setFormValue] = useState(resumeJson[sectionKey]?.data?.map(item => item ?? "") || []);
    const dispatch = useDispatch();
    // useEffect(() => {
    //     let intialValue = resumeJson[sectionKey]?.data?.map(item => item ?? "") || [];
    ...
    const handleExpand = () => {
        ...
        setExpanded(expanded === sectionKey ? "" : sectionKey);
    };
    ...
    useEffect(() => {
        setResumeJson({ ...resumeJson, [sectionKey]: { ...resumeJson[sectionKey], data: formValue } });
    }, [formValue]);
    ...
    const handleDeleteSection = () => {
        dispatch({ type: REMOVE_EDITOR_SECTION, payload: sectionKey });
    ...
    const handleBlurSectionName = (e) => {
        setResumeJson({ ...resumeJson, [sectionKey]: { ...resumeJson[sectionKey], sectionName: e.target.value } });
        setEditSectionName(false);
    }
    ...
    return (
        <div className="re-section">
                    ...
                    <>
                        {editSectionName ? (
                            <input
                                ...
                                defaultValue={resumeJson[sectionKey]?.sectionName}
                                onBlur={(e) => handleBlurSectionName(e)}
                                onKeyDown={(e) => e.key === "Enter" && handleBlurSectionName(e)}
                            />
                        ) : (
                            <h6>{resumeJson[sectionKey]?.sectionName}</h6>
                        )}
                        <div className="re-section-head-actions">
                            <button
                                className="icon-btn"
                            ...
                            <span className={`expand-icon ${expanded === sectionKey ? "expanded" : ""}`}>
                                <ArrowDownIcon />
                            </span>
                        </div>
                    </>
            ...
            {expanded === sectionKey && (
                <div className="re-section-content" data-editor-item-id={"re$" + sectionKey + "$data"}>
...
const REGenericItem = ({ sectionKey, genericItem, itemIndex, handleBulletChange, handleDeleteBullet }) => {

    // const textAreaRef = useRef(null);

    const editorItemId = "re$" + sectionKey + "$" + itemIndex;
    ...
    return (
        <div className="bullet-point" data-editor-item-id={editorItemId}>
            {/* <textarea
                ref={textAreaRef}
                value={genericItem}
                onChange={(e) => handleBulletChange(e.target.value, itemIndex)}
                autoFocus={!genericItem}
                onBlur={(e) => handleBulletChange(e.target.value, itemIndex)}
                rows={1}
            /> */}
            <ResumeJoditField
                content={genericItem}
                onBlur={val => handleBulletChange(val, itemIndex)}
                forceEditorFocus={!genericItem || editorOpenFromPreview}
            />
