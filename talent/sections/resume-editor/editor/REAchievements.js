...
export default function REAchievements({ expanded, setExpanded, resumeJson, setResumeJson }) {
    const [formValue, setFormValue] = useState(resumeJson.achievements?.data?.map(item => item ?? "") || []);
    const dispatch = useDispatch();
    // useEffect(() => {
    //     let intialValue = resumeJson.achievements?.data?.map(item => item ?? "") || [];
    ...
    const handleExpand = () => {
        ...
        setExpanded(expanded === "achievements" ? "" : "achievements");
    };
    ...
    useEffect(() => {
        setResumeJson({ ...resumeJson, achievements: { ...resumeJson.achievements, data: formValue } });
    }, [formValue]);
    ...
    const handleDeleteSection = () => {
        dispatch({ type: REMOVE_EDITOR_SECTION, payload: "achievements" });
    ...
    const handleBlurSectionName = (e) => {
        setResumeJson({ ...resumeJson, achievements: { ...resumeJson.achievements, sectionName: e.target.value } });
        setEditSectionName(false);
    }
    ...
    return (
        <div className="re-section">
                    ...
                    <>
                        {editSectionName ? (
                            <input
                                ...
                                defaultValue={resumeJson.achievements?.sectionName}
                                onBlur={(e) => handleBlurSectionName(e)}
                                onKeyDown={(e) => e.key === "Enter" && handleBlurSectionName(e)}
                            />
                        ) : (
                            <h6>{resumeJson.achievements?.sectionName}</h6>
                        )}
                        <div className="re-section-head-actions">
                            <button
                                className="icon-btn"
                            ...
                            <span className={`expand-icon ${expanded === "achievements" ? "expanded" : ""}`}>
                                <ArrowDownIcon />
                            </span>
                        </div>
                    </>
            ...
            {expanded === "achievements" && (
                <div className="re-section-content" data-editor-item-id={"re$achievements$data"}>
...
const REAchievementItem = ({ achievementItem, itemIndex, handleBulletChange, handleDeleteBullet }) => {

    const textAreaRef = useRef(null);

    const editorItemId = "re$achievements$" + itemIndex;
